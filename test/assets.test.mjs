...
const walkDir = 'extension/assets/nox/walk';
...
describe('Nox V3 walking assets', () => {
    it('contains exactly V1 walk WebP frames 0.webp..15.webp', () => {
        const expected = Object.keys(expectedHashes);
        assert.deepEqual(readdirSync(walkDir).sort(numericSort), expected);
    });

    it('contains no other asset files or folders', () => {
        assert.deepEqual(readdirSync('extension/assets'), ['nox']);
        assert.deepEqual(readdirSync('extension/assets/nox'), ['walk']);
        assert.equal(statSync(walkDir).isDirectory(), true);
    });

    it('preserves the approved V1 walking asset hashes exactly', () => {
        for (const [name, expectedHash] of Object.entries(expectedHashes))
            assert.equal(sha256(join(walkDir, name)), expectedHash, name);
    });
});
